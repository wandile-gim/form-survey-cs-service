...
type TaskQuery struct {
	...
	withTaskRecord *TaskRecordQuery
...
// QueryTaskRecord chains the current query on the "task_record" edge.
func (tq *TaskQuery) QueryTaskRecord() *TaskRecordQuery {
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withTaskRecord: tq.withTaskRecord.Clone(),
...
// WithTaskRecord tells the query-builder to eager-load the nodes that are connected to
// the "task_record" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithTaskRecord(opts ...func(*TaskRecordQuery)) *TaskQuery {
	query := (&TaskRecordClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withTaskRecord = query
	return tq
}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withTaskRecord; query != nil {
		if err := tq.loadTaskRecord(ctx, query, nodes, nil,
			func(n *Task, e *TaskRecord) { n.Edges.TaskRecord = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadTaskRecord(ctx context.Context, query *TaskRecordQuery, nodes []*Task, init func(*Task), assign func(*Task, *TaskRecord)) error {
	...
	query.Where(predicate.TaskRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.TaskRecordColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.task_task_record
		if fk == nil {
			return fmt.Errorf(`foreign-key "task_task_record" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "task_task_record" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
